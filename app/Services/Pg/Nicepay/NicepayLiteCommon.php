...
use const HTTP_SSL_PORT;
use const HTTP_SSL_SERVER;
use const READ_TIMEOUT_ERR;
use const TIMEOUT_READ;

define("PROGRAM", "NicepayLite");
define("LIB_LANG", "PHP");
define("VERSION", "1.3.0");
define("BUILDDATE", "20171215");
    ...
    if ("CARD" == $payMethod) {
        ...
        if ("BANK" == $payMethod) {
            ...
            if ("VBANK" == $payMethod) {
                ...
                if ("CELLPHONE" == $payMethod) {
                    $svcCd = "05";
                } else {
                    if ("CPBILL" == $payMethod) {
                        $svcCd = "06";
                    } else {
                        if ("VBANK_BULK" == $payMethod) {
                            $svcCd = "03";
                        } else {
                            if ("CASHRCPT" == $payMethod) {
                                $svcCd = "04";
                            } else {
                                if ("GIFT_SSG" == $payMethod) {
                                    // SSG머니
                                    $svcCd = "21";
                                } else {
                                    if ("SSG_BANK" == $payMethod) {
                                        // SSG 은행직불
                                        $svcCd = "24";
                                    } else {
...
class HttpClient
{
    ...
    public function __construct($ssl, $ReqHost, $ReqPort)
    {
    ...
    public function HttpConnect($NICELog)
    {
    ...
    public function HttpRequest($uri, $data, $NICELog)
    {
        $this->headers = "";
        $this->body = "";
