...
namespace App\Services\Pg\Uplus;
...
//        }
        ...
        $logDir = env('DEVELOPMENT') == 'real' ? $this->config["log_dir"] : $this->config["dev_log_dir"];

        $this->log_file = $logDir . "/log_" . date("Ymd") . ".log";
...
//		$this->log("session id = ".session_id().$_SESSION['tx_counter'], LGD_LOG_FATAL);
    ...
    function TX($bRollbackOnError = true)
    {
        ...
        if ($result == false) {
            ...
            $this->log(
                "TX failed: res code = " . $this->response_code . "; msg = " . $this->response_msg,
                LGD_LOG_FATAL
            );
            ...
            if ($http_res_code < 200 || $http_res_code >= 300) {
                ...
                if (($this->response_array == false) || (strlen($this->response_array["LGD_RESPCODE"]) == 0)) {
                    ...
                    $this->log(
                        "Response Code=[" . $this->response_code . "], Msg=[" . iconv(
                            "utf-8",
                            "euc-kr",
                    ...
                    for ($i = 0; $i < $this->Response_Count(); $i++) {
                        foreach ($keys as $name) {
                            $this->log(
                                "Response (" . $name . ", " . $i . ") = " . $this->Response($name, $i),
                                LGD_LOG_DEBUG
                            );
                        }
                    }
    ...
    function Report_TX()
    {
        ...
        if ($result == false) {
            set_curl_error();
            $this->log(
                "Reporting failed: res code = " . $this->response_code . "; msg = " . $this->response_msg,
                LGD_LOG_ERROR
            );
        ...
        $response_msg = iconv("euc-kr", "utf-8", $response_array["LGD_RESPMSG"]);
    ...
    function send_post_data($url, $postdata, $ip = null, $timeout = 10)
    {
        ...
        if (is_array($postdata)) {
            foreach ($postdata as $key => $value) {
                $post_array[] = urlencode((string)$key) . "=" . urlencode((string)$value);
