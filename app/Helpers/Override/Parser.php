...
class Parser
{
    ...
    private function doParse(string $value, int $flags)
    {
        ...
        do {
            ...
            if ('-' === $this->currentLine[0] && self::preg_match(
                ...
                if (isset($values['value']) && '&' === $values['value'][0] && self::preg_match(
                ...
                } elseif (!isset($values['value']) || '' == trim($values['value'], ' ') || 0 === strpos(
                        ltrim($values['value'], ' '),
                        '#'
                    )) {
                    ...
                    if (
                        ...
                        if ($this->isNextLineIndented()) {
                            $block .= "\n" . $this->getNextEmbedBlock(
                                    $this->getCurrentLineIndentation() + strlen($values['leadspaces']) + 1
                                );
                        }
                ...
                self::preg_match(
                    '#^(?P<key>(?:![^\s]++\s++)?(?:' . Inline::REGEX_QUOTED_STRING . '|(?:!?!php/const:)?[^ \'"\[\{!].*?)) *\:(( |\t)++(?P<value>.+))?$#u',
                    rtrim($this->currentLine),
                    $values
                )
                ...
                $context = 'mapping';
                if (Arr::has($values, 'value')) {
                    preg_match('/\${(.*)}/', $values['value'], $match);
                    if (empty($match) === false && env($match[1])) {
                        $values['value'] = env($match[1]);
                    }
                }
                ...
                if (!is_string($key) && !is_int($key)) {
                    throw new ParseException(
                        (is_numeric(
                            $key
                        ) ? 'Numeric' : 'Non-string') . ' keys are not supported. Quote your evaluable mapping keys instead.',
                        $this->getRealCurrentLineNb() + 1,
                        $this->currentLine
                    );
                }
                ...
                if ('<<' === $key && (!isset($values['value']) || '&' !== $values['value'][0] || !self::preg_match(
                            '#^&(?P<ref>[^ ]+)#u',
                            $values['value'],
                            $refMatches
                        ))) {
                ...
                } elseif ('<<' !== $key && isset($values['value']) && '&' === $values['value'][0] && self::preg_match(
                        self::REFERENCE_PATTERN,
                        $values['value'],
                        $matches
                    )) {
                ...
                if ($mergeNode) {
                    // Merge keys
                } elseif (!isset($values['value']) || '' === $values['value'] || 0 === strpos(
                        $values['value'],
                        '#'
    ...
    private function getNextEmbedBlock(int $indentation = null, bool $inSequence = false): string
    {
        ...
        while ($this->moveToNextLine()) {
            ...
            if ($isItUnindentedCollection && !$this->isCurrentLineEmpty() && !$this->isStringUnIndentedCollectionItem(
                ) && $newIndent === $indent) {
                $this->moveToPreviousLine();
                break;
            }
    ...
    private function parseValue(string $value, int $flags, string $context): mixed
    {
        ...
        if (in_array($value[0], ['!', '|', '>'], true) && self::preg_match(
                '/^(?:' . self::TAG_PATTERN . ' +)?' . self::BLOCK_SCALAR_HEADER_PATTERN . '$/',
                $value,
                $matches
            )) {
        ...
        try {
            ...
            switch ($value[0] ?? '') {
                ...
                case "'":
                    ...
                    if (isset($this->currentLine[$cursor]) && preg_replace(
                            '/\s*(#.*)?$/A',
                            '',
                            substr($this->currentLine, $cursor)
                        )) {
                        throw new ParseException(
                            sprintf('Unexpected characters near "%s".', substr($this->currentLine, $cursor))
                        );
                    }
                ...
                default:
                    ...
                    if ('mapping' === $context && is_string(
                            $parsedValue
                        ) && '"' !== $value[0] && "'" !== $value[0] && '[' !== $value[0] && '{' !== $value[0] && '!' !== $value[0] && false !== strpos(
                            $parsedValue,
                            ': '
                        )) {
                        throw new ParseException(
    ...
    private function isCurrentLineComment(): bool
    {
        ...
        $ltrimmedLine = '' !== $this->currentLine && ' ' === $this->currentLine[0] ? ltrim(
            $this->currentLine,
            ' '
        ) : $this->currentLine;
    ...
    {
        if ('' === $value || '!' !== $value[0] || 1 !== self::preg_match(
                '/^' . self::TAG_PATTERN . ' *( +#.*)?$/',
                $value,
                $matches
            )) {
